#based on multilexnorm/scripts/normEval.py + added OR and UR
...
def evaluate(raw, gold, pred, ignCaps=False, verbose=False):
    ...
    for sentRaw, sentGold, sentPred in zip(raw, gold, pred):
        ...
        for wordRaw, wordGold, wordPred in zip(sentRaw, sentGold, sentPred):
            ...
            if wordRaw == wordGold and wordGold != wordPred:
                false_positive += 1
            if wordRaw == wordPred and wordRaw != wordGold:
                modified += 1
            if wordRaw == wordGold:
                non_norm += 1
            elif verbose:
    ...
    over_norm = false_positive / total
    under_norm = modified / total    

    print('Baseline acc.(LAI): {:.2f}'.format(lai * 100)) 
    print('Accuracy:           {:.2f}'.format(accuracy * 100)) 
    print('ERR:                {:.2f}'.format(err * 100))
    print('OR:                 {:.2f}'.format(over_norm * 100))
    print('UR:                 {:.2f}'.format(under_norm * 100))
